...
export async function handleTelegramMessage(openai: OpenAI, redis: Redis, telegram: TelegramBot, msg: TelegramBot.Message, redisMethods: RedisMethods, claude: Claude) {
  ...
  const imageUrl = "https://res.cloudinary.com/dwrwxeoih/image/upload/v1717701583/pr3qggcdy7bweqbeupaa.jpg";
  const videoUrl = "https://res.cloudinary.com/dwrwxeoih/video/upload/v1717701734/env3nodoknzmh9zxynps.mp4";

  if (msg.text === '/start') {
    await telegram.sendPhoto(id, imageUrl);
    return;
  }
    ...
    const initialGeneration = [
      {
        ...
        telegram:  t.me/Pepe_meme_gpt_bot
...
‍
        ...
        Roadmaps are for kids, yes kids, real pepe adults only need vibes and wives. If you need help feel free to ask the rizz god PEPEPEPEPEPEPEPEPEPEPEPEPEPEPEPE for tips, tricks, and tokens. We lub people who stand up for themselves and do real shit in lyf. You only lyf once, but okay heres alpha:
